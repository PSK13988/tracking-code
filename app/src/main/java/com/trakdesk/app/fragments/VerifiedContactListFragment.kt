...
class VerifiedContactListFragment : BaseFragment(), View.OnClickListener,
    ...
    companion object {
        val TAG = VerifiedContactListFragment::class.java.name

        fun newInstance(): VerifiedContactListFragment {
            return VerifiedContactListFragment()
        }
    }
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_verified_contact_list, container, false)
    }
                    ...
                    contactsAdapter = context?.let { it1 ->
                        ...
                        for (item in aToz) {
                            val result = contactResponse.data!!.filter { contactItem ->
                                println("Verified Contacts : ${contactItem.name} = ${contactItem.verified}")
                                contactItem.verified and (contactItem.name.startsWith(item) or contactItem.name.startsWith(
